...
export default function CommunityPage() {
  ...
  async function vote(postId: string, delta: 1 | -1) {
    ...
    mutate(
      async (prev) => {
        if (!prev) return prev;
        const next = { ...prev, posts: prev.posts.map(p => {
          if (p._id !== postId) return p;
          // Compute next myVote and votes locally
          const prevMy: -1 | 0 | 1 = p.myVote ?? 0;
          let nextMy: -1 | 0 | 1 = prevMy === delta ? 0 : delta; // toggle if same arrow pressed
          const diff = nextMy - prevMy;             // how much to add to votes
          return { ...p, myVote: nextMy, votes: (p.votes ?? 0) + diff };
        }) as Post[] };
        // Fire request but ignore result errors (we'll revert on throw)
  ...
  function Pill({ children }: { children: React.ReactNode }) {
    return (
      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border border-slate-200 bg-slate-50 text-slate-600">
        {children}
      </span>
    );
  }
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 md:py-16">
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            posts.map((p) => (
              <PostCard key={p._id} post={p} onVote={(d) => vote(p._id, d)} />
            ))
          )}
        </div>
      </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            {post.createdAt && <span className="text-xs text-slate-500">• {new Date(post.createdAt).toLocaleString()}</span>}
